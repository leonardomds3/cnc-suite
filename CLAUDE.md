...
- `estudio_cnc.html` — app mobile-first com três abas (PROJETO / 3D / CÓDIGO). A aba PROJETO tem uma tela 2D em SVG onde as operações ("blocos") são posicionadas e arrastadas direto sobre uma vista de topo do bloco de material, além de um painel inspetor para editar os parâmetros do bloco selecionado. Preservado como conceito; não recebe as extrações de módulo abaixo além do `ui.css` — ver "Interface ativa" no STATUS.md.
