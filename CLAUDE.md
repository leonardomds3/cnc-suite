...
`DEFS` é um objeto indexado pelo id da operação (`face`, `bolsaCon`, `furosL`, `furosC`, além das **fichas de estratégia** registradas do `estrategias.js` e de qualquer macro personalizada cadastrada pelo usuário). Cada entrada define um tipo de operação de usinagem:
