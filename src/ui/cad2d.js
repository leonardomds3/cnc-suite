...
function cadCarregar(data){
 ...
 if(data){
  ...
  const ids=new Set();
  const entities=data.entities.map(e=>{if(!Number.isSafeInteger(e.id)||e.id<1||ids.has(e.id)||!cadValid(e))throw Error('Entidade de desenho inválida.');ids.add(e.id);const clean={id:e.id,type:e.type};CAD_FIELDS[e.type].forEach(([k])=>clean[k]=e[k]);if(e.link!=null)clean.link=e.link;if(e.desenho2D)clean.desenho2D=e.desenho2D;return clean;});
